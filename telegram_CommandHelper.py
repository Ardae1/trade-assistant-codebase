...
def analyze_coin(update: Update, context):
    ...
    if coin_name.endswith('USDT') or coin_name.endswith('BTC'):
        update.message.reply_text(f'Please remove USDT or BTC at the end and try again..')
        return
    else:    
        update.message.reply_text(f'analyzing for coins: {coin_name}USDT and {coin_name}BTC ...')
    try:
...
def main() -> None:
    ...
    task = CommandHandler('add_coin',add_coin)
    task2 = CommandHandler('analyze_coin', analyze_coin)

    db.add_handler(task)
    db.add_handler(task2)
